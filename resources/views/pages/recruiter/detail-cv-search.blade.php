...
<div class="container-fluid">
  <div class="container">
    <div class="row">
      <div class="col-md-8">
        @if(Session::get('page_redirect') == 'list-cv-save')
        <a href="{{ route('recruiter.list_cv_save') }}" class="btn btn-lg btn-success">Quay Lại</a>
        @elseif(Session::get('page_redirect') == 'list-cv-view')
        <a href="{{ route('recruiter.list_cv_view') }}" class="btn btn-lg btn-success">Quay Lại</a>
        @elseif(Session::get('page_redirect') == 'search')
        <a href="{{route('recruiter.cv_search')}}" class="btn btn-lg btn-success">Quay Lại</a>
        @endif
      </div>
      <div class="col-md-4">
...
<div class="container-fluid published-recuitment-wrapper mt-3 mb-3">
  <div class="container published-recuitment-content">
    <div class="row">
            ...
            <div class="card-body">
                ...
                <div class="col-6 text-left" >
                    ...
                    <div class="row">
                        ...
                        <div class="col-9" >@if($existCV_DaDungDiemTK) {{$ungVien->email}} @else [protected_info@gmail.com] @endif</div>
                    </div>
                    <hr>
                    <div class="row ">
                        <div class="col-3" ><strong class="font-weight-bold">SĐT:</strong></div>
                        <div class="col-9">@if($existCV_DaDungDiemTK) {{$ungVien->sdt}} @else [protected_number_phone] @endif</div>
                    </div>
                </div>
                <hr>
                </div>
        ...
        <div class="card mt-3">
            ...
            @if($existCV_DaDungDiemTK) 
                <object width="100%" height="800px" type="application/pdf" data="{{ URL::asset('public/cv/'.$cv->tenfile) }}?#zoom=85&scrollbar=0&toolbar=0&navpanes=0">
                    <p>Không có hoặc không tìm thấy File CV</p>
                </object>
            @else 
                [protected_cv] 
            @endif
      ...
      <!-- Side bar -->
        @include('layout.recruiter.sidebar-detail-cv-search')
        <!-- End Side bar -->
    </div>
  </div>
</div>
...
<script type="text/javascript">
  $('#msgErr').show()
  setTimeout(function() {
    $("#msgErr").fadeTo(1000, 400).slideUp(400, function(){
        $("#msgErr").fadeOut();
    });
  }, 1000);
</script>
